...
- **固定编码链接**：每个文件自动生成不可变的随机编码链接，隐藏真实路径；移动 / 覆盖 / 刷新后链接不变，旧路径访问继续兼容；
...
- 公开首页：文件列表（名称、大小、更新时间、来源徽章：本地上传 / 远程拉取 / 手动输入、备注内容、固定编码链接）
...
- 下方是**文件列表**：点「复制」复制**固定编码链接**（隐藏路径，移动文件也不失效）；点文件夹标签按目录看文件；有备注的文件会直接显示备注内容（备注仅管理员可编辑）。
...
文件列表「操作」列（验证 Token 后显示）：打开 / 复制（固定编码链接）/ 备注；管理操作（覆盖更新 / 从来源更新 / 移动 / 复制路径链接 / 删除）收在「⋯」菜单里。顶部可下载**通用 upload.bat**。

## upload.bat

验证 Token 后下载一个**通用**脚本（脚本里已写好你的部署地址和 Token，**不要发给别人**）。

- 把文件直接拖到 `upload.bat` 上（路径会自动填入）；或双击运行，按提示粘贴本地文件路径。
- 云端路径自动补全（也可用第二个参数传入路径）：
  - 直接回车 → `auto/文件名`；
  - 目录以 `/` 结尾 → `目录/文件名`（例如 `sub/` → `sub/文件名`）；
  - 只写名字不写后缀 → 自动补本地文件后缀（例如 `sub/report` → `sub/report.txt`）。
- 上传成功后会显示接口返回的固定链接（推荐）和路径链接，并**自动打开浏览器预览**。

脚本使用 Windows 自带的 PowerShell 发送原始文件字节，无需额外安装 `curl.exe`。页面更新后请重新下载脚本，旧脚本不会自动升级。
...
> 给其他项目/脚本对接的完整示例（curl、PowerShell、Python、Node.js、Go）：[docs/upload-api.md](docs/upload-api.md)


| 方法 | 路径 | 说明 | 鉴权 |
| --- | --- | --- | --- |
| GET | `/api/files` | 文件列表（名称 / 大小 / 时间 / 来源 / 备注 / 固定编码 / 路径链接） | 公开 |
| GET | `/<key>` | 读取文件内容 | 公开 |
| GET | `/<code>` | 用固定编码读取文件（隐藏路径，下载文件名为编码） | 公开 |
...
- `__meta/`、`__id/` 为系统保留前缀，禁止写入；云端路径禁止 `..`、反斜杠和危险字符
