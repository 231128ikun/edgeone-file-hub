...
- **URL 反代**：主页输入任意 http/https 网址，自动生成 `/api/proxy?url=...` 链接，可复制或直接打开读取远端内容，**只转发不保存**；
...
- 公开首页：文件列表（名称、大小、更新时间、来源徽章：本地上传 / 远程拉取 / 手动输入）
...
- 上方是 **URL 反代**：输入网址（不带 `https://` 会自动补）→ 点「打开」直接访问，或点「复制」把反代链接发给别人。只转发不保存，不会出现在文件列表里。
- 下方是**文件列表**：点「复制」复制公开链接；点文件夹标签按目录看文件。
...
文件列表「操作」列（验证 Token 后显示）：复制链接 / 更新 / 删除；顶部可下载**通用 upload.bat**。
...
| 方法 | 路径 | 说明 | 鉴权 |
| --- | --- | --- | --- |
| GET | `/api/files` | 文件列表（名称 / 大小 / 时间 / 来源 / 链接） | 公开 |
| GET | `/<key>` | 读取文件内容 | 公开 |
...
- 普通用户只能读文件和偶尔用 URL 反代；写入 / 删除 / 拉取保存全部由后端校验 Token 管住，前端隐藏按钮只是简化界面
